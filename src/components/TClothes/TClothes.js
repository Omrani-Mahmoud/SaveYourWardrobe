...
export default function TClothes() {
  ...
  const fetchStores = async () => {
    const datatFromDataBase = await fetch(
      `http://code-beast.herokuapp.com/store`
    );
  ...
  const fetchSubscribedStoreItems = async () => {
    const datatFromDataBase = await fetch(
      `http://code-beast.herokuapp.com/userStoreItemsToReact/`+window.localStorage.getItem("connectedUserID")
    );
    const data = await datatFromDataBase.json();
  ...
  async function handleChildClick(favorite) {
    await axios.patch("http://code-beast.herokuapp.com/updateUserStoreItemsToReact/"+window.localStorage.getItem("connectedUserID")+"/"+favorite.action+"/"+favorite.item_id)
        .then(res=>{
            console.log(res)
        })
        .catch(err=>{
